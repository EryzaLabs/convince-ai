...
def _extract_content(content: str) -> str:
    ...
    # Strip any leading timestamps generated by LLM
    content = re.sub(r'^\[?\d{4}-\d{2}-\d{2}[T\s]\d{2}:\d{2}:\d{2}(?:\.\d+)?Z?\]?\s*', '', content)
...
def split_response_messages(text):
    ...
    # Clean timestamps from each split bubble
    cleaned_parts = []
    for p in parts:
        cleaned = re.sub(r'^\[?\d{4}-\d{2}-\d{2}[T\s]\d{2}:\d{2}:\d{2}(?:\.\d+)?Z?\]?\s*', '', p)
...
TIMESTAMPS: You will see timestamps formatted as [YYYY-MM-DD HH:MM:SS] at the beginning of messages in the conversation history. These are injected for timing context ONLY. You MUST NOT include, prepend, or mimic timestamps or date brackets in your own response messages.
...
- TIMESTAMPS: THE MESSAGE HISTORY CONTAINS TIMESTAMPS FOR YOUR CONTEXT ONLY. DO NOT PREPEND, WRITE, OR IMITATE TIMESTAMPS, DATE LABELS, OR BRACKETS IN YOUR OUTPUTS. YOUR MESSAGES MUST BE PURE TEXT.
